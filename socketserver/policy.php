...
require_once(dirname(__FILE__).'/conf/includes.php');
...
function send_Message($allclient, $socket, $buf) {
   foreach($allclient as $client) {
		...
		print "<br />request: ".htmlspecialchars($buf);
		if(strpos($buf, 'policy-file-request') !== false)
		{
		  $resp = "<?xml version=\"1.0\"?><cross-domain-policy><allow-access-from domain=\"*\" to-ports=\"".SERVER_PORT."\" /></cross-domain-policy>\0";
		  print "<br />response: ".htmlspecialchars($resp)."<br>";
